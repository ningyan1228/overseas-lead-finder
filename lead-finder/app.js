(() => {
  ...
  $("#taskForm").addEventListener("submit", async (event) => { event.preventDefault(); const data = Object.fromEntries(new FormData(event.currentTarget)); ["maxKeywords", "maxResults", "minScore"].forEach((key) => { data[key] = Number(data[key]); }); try { await api("/search-tasks", { method: "POST", body: JSON.stringify(data) }); await loadTasks(); message("任务已加入队列，worker 会自动处理。"); } catch (error) { message(error.message); } });
